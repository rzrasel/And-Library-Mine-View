...
public class RSVPVideoView extends SurfaceView implements RSVPMediaController.MediaPlayerControl, RSVPOrientationDetector.OrientationChangeListener {
    private String TAG = "RSVPVideoView";
    ...
    private RSVPMediaController mMediaController;
    ...
    private RSVPOrientationDetector mOrientationDetector;
    private VideoViewCallback videoViewCallback;

    public RSVPVideoView(Context context) {
        this(context, null);
    }

    public RSVPVideoView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public RSVPVideoView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;
        TypedArray a = mContext.obtainStyledAttributes(attrs, R.styleable.RSVPVideoPlayer, 0, 0);
        mFitXY = a.getBoolean(R.styleable.RSVPVideoPlayer_rsvp_fitXY, false);
        mAutoRotation = a.getBoolean(R.styleable.RSVPVideoPlayer_rsvp_autoRotation, false);
        a.recycle();
        initVideoView();
    }
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(RSVPVideoView.class.getName());
    }
    ...
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            info.setClassName(RSVPVideoView.class.getName());
        }
    }
    ...
    @Override
    public void onOrientationChanged(int screenOrientation, RSVPOrientationDetector.Direction direction) {
        if (!mAutoRotation) {
            return;
        }

        if (direction == RSVPOrientationDetector.Direction.PORTRAIT) {
            setFullscreen(false, ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);
        } else if (direction == RSVPOrientationDetector.Direction.REVERSE_PORTRAIT) {
            setFullscreen(false, ActivityInfo.SCREEN_ORIENTATION_SENSOR_PORTRAIT);
        } else if (direction == RSVPOrientationDetector.Direction.LANDSCAPE) {
            setFullscreen(true, ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
        } else if (direction == RSVPOrientationDetector.Direction.REVERSE_LANDSCAPE) {
    ...
    public void setMediaController(RSVPMediaController controller) {
...
//                    }
    ...
    private void enableOrientationDetect() {
        if (mAutoRotation && mOrientationDetector == null) {
            mOrientationDetector = new RSVPOrientationDetector(mContext);
            mOrientationDetector.setOrientationChangeListener(RSVPVideoView.this);
